...
import { getOrderDetail, listAllOrders } from '@/modules/order/api/orderApi'
import { createProductStockTransaction } from '@/modules/inventory/api/inventoryApi'
...
import { aggregateShippableAmount } from '@/modules/inventory/utils/shippableOrders'

// Stock in/out flow: dialog + form state, the可出貨訂單 loader (listAllOrders → filter
// active → getOrderDetail per order → aggregate shippable amount per product), order
// selection, validation, and the create-then-reload save. `loadProducts` reloads the
// inventory after a successful save; `statusMessage`/`errorMessage` are the page refs.
  ...
  const loadTransactionOrders = async (product) => {
    ...
    try {
      ...
      transactionOrders.value = details.flatMap((order) => {
        const amount = aggregateShippableAmount(order, product.id)
